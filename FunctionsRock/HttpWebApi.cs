...
namespace Snoopy.Function
{
    public class HttpWebApi
    {
        ...
        [Function("GetStudentCountsBySchool")]
        public HttpResponseData GetStudentsCountBySchool([HttpTrigger(AuthorizationLevel.Function, "get", Route = "countbyschool")] HttpRequestData req)
        {
            _logger.LogInformation("C# HTTP GET/posts trigger function processed a request inGetStudents().");

            var students = _context.Students.ToArray();

            var countsBySchool = students
            ...
            var response = req.CreateResponse(HttpStatusCode.OK);
            response.Headers.Add("Content-Type", "application/json");
            response.WriteString(JsonConvert.SerializeObject(countsBySchool));
